...
const {
	...
	packPoint,
	unpackPoint
} = require("./node_modules/circomlib/src/babyjub.js");
...
exports.G = new BabyJubPoint(F.e("16540640123574156134436876038791482806971768689494387082833631921987005038935"), F.e("20819045374670962167435360035096875258406992893633759881276124905556507972311"));
class Fr {
	constructor(n) {
		if (typeof n == "bigint") {
...
function randFr() {
	...
	return new Fr(BigInt(utils.leBuff2int(buff)));
}

function keyGen(secret) {
	let G = new BabyJubPoint(F.e("16540640123574156134436876038791482806971768689494387082833631921987005038935"), F.e("20819045374670962167435360035096875258406992893633759881276124905556507972311"));
	if (typeof secret === 'bigint') {
		const secretValInField = new Fr(secret);
		const pointOnCurve = G.mul(secretValInField);
		const compressedPoint = new BabyJubPoint(pointOnCurve.x, pointOnCurve.y);
		const compressedPointInt = utils.leBuff2int(compressedPoint.compress());
		return {
			'infield': secretValInField.n.toString(),
			'Public_point': pointOnCurve,
			'Public_key': compressedPointInt
		};
	} else {
...
function keyGenRand() {
	...
	return {
		'Secret': buffToBigint,
		'Public_point': keyPointOnCurve,
		'Public_key': compressedPointInt,
		'Secret_in_field': randomValInField.n
	};
}
